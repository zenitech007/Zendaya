...
type Tier = "ultra" | "standard" | "reduced" | "minimal";

function fpsTier(fps: number): Tier {
  if (fps >= 120) return "ultra";
  if (fps >= 60)  return "standard";
  if (fps >= 45)  return "reduced";
  return "minimal";
}

class AdaptiveAudioControllerSingleton {
  private currentTier: Tier = "standard";
  ...
  private _onFpsChange(fps: number) {
