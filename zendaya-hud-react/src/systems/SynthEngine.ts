...
import { AudioManager, type LayerName } from "./AudioManager";
import type { AiState } from "../store/zendayaStore";

type InteractionType = "hover" | "click" | "confirm" | "open" | "close" | "dock" | "error";
type AlertType = "info" | "warning" | "critical" | "error";
type SweepType = "expand" | "contract" | "minimize" | "restore" | "scene";

class SynthEngineSingleton {
  private get ctx() { return AudioManager.ctx; }

  // ── Connect a chain of nodes → layer gain ────────────────────────────────
  private connect(node: AudioNode, layer: LayerName) {
    const layerGain = AudioManager.getLayerGain(layer);
    if (layerGain) node.connect(layerGain);
  }

  // ── Utility: create a master gain for a synth voice ──────────────────────
  private makeVoiceGain(layer: LayerName, initial = 0): GainNode | null {
  ...
  private _dock() {
    ...
    [{ f: 90, type: "sine" as OscillatorType, dur: 0.12, amp: 0.28 },
     { f: 1200, type: "square" as OscillatorType, dur: 0.05, amp: 0.10 }]
  ...
  playTransitionSweep(type: SweepType) {
    switch (type) {
      case "expand":   this._expandSweep();   break;
      case "contract": this._contractSweep(); break;
      case "minimize": this._minimizeSweep(); break;
      case "restore":  this._restoreSweep();  break;
      case "scene":    this._sceneSweep();    break;
    }
  }

  private _expandSweep() {
    const ctx = this.ctx; if (!ctx) return;
  ...
  playOrbState(state: AiState | "idle") {
    switch (state) {
      case "idle":      this._orbIdle();      break;
      case "listening": this._orbListening(); break;
      case "thinking":  this._orbThinking();  break;
      case "speaking":  this._orbSpeaking();  break;
    }
  }
  ...
  private _orbIdle() {
    ...
    const osc = ctx.createOscillator();
    osc.type = "sine"; osc.frequency.value = 55; // 55 Hz — sub-bass foundation
  ...
  private _orbThinking() {
    ...
    modFreqs.forEach((f, i) => {
      ...
      lfo.type = "sine"; lfo.frequency.value = 4 + i * 1.5;
      const lfoGain = ctx.createGain(); lfoGain.gain.value = f * 0.08;
      ...
      osc.stop(start + 0.55); lfo.stop(start + 0.55);
    });
  }

  private _orbSpeaking() {
    const ctx = this.ctx; if (!ctx) return;
    ...
    const filter = ctx.createBiquadFilter();
    filter.type = "peaking"; filter.frequency.value = 1200;
    filter.gain.value = 6; filter.Q.value = 1.5;
    osc.connect(filter); filter.connect(g);
    const t = ctx.currentTime;
    g.gain.setValueAtTime(0, t);
  ...
  playAlert(type: AlertType) {
    switch (type) {
      case "info":     this._alertInfo();     break;
      case "warning":  this._alertWarning();  break;
      case "critical": this._alertCritical(); break;
      case "error":    this._alertError();    break;
    }
  }
  ...
  private _alertCritical() {
    ...
    osc.connect(dist); dist.connect(filter); filter.connect(g);
    const t = ctx.currentTime;
  ...
  private _makeDistCurve(amount: number): any {
    const n = 256;
