...
class SpatialAudioEngineSingleton {
  ...
  private driftNode: StereoPannerNode | null = null;
  private driftRafId = 0;
  ...
  updatePannerPosition(panner: PannerNode, x: number, y: number, z: number) {
    const ctx = AudioManager.ctx;
  ...
  updateListener(
    ...
    if (l.positionX) {
      ...
      // Older API fallback
      (l as any).setPosition(posX, posY, posZ);
      (l as any).setOrientation(fwdX, fwdY, fwdZ, upX, upY, upZ);
    }
  }

  // ── Subtle slow stereo drift (spec: "floating, atmospheric, holographic") ─
  /** Start a persistent slow drift on a StereoPannerNode */
  startOrbDrift(): StereoPannerNode | null {
    const ctx = AudioManager.ctx;
    if (!ctx) return null;
    if (this.driftNode) return this.driftNode;
  ...
  stopOrbDrift() {
    ...
    if (this.driftNode) {
      try { this.driftNode.disconnect(); } catch { /* */ }
      this.driftNode = null;
    }
  }
  ...
  createWideStereoSend(opts: { spread?: number } = {}): {
