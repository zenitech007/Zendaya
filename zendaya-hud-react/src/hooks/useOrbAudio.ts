...
export function useOrbAudio() {
  ...
  useEffect(() => {
    const unsub = useZendaya.subscribe((state) => {
      const ai = state.ai;
      if (ai === prevAiRef.current) return;
      ...
      if (shimmerOscRef.current) {
        ...
        shimmerOscRef.current.frequency.setTargetAtTime(
          freqMap[ai] ?? 2400, t, 0.3
        );
      }
  ...
  useEffect(() => {
    const unsub = useZendaya.subscribe((state) => {
      ...
      const ctx = AudioManager.ctx;
      if (!ctx || !shimmerGainRef.current) return;
...
function _startScanTicks() {
  ...
  const tick = () => {
    ...
    osc.connect(g); g.connect(layerGain);
    const t = ctx.currentTime;
