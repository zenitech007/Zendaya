...
const VALID_AI = ["idle", "listening", "thinking", "speaking", "error"];
...
function dispatchAction(action, payload) {
    ...
    switch (action) {
        ...
        case "close_map":
            ...
            break;
        case "open_module": {
            const valid = ["map", "calculator", "clock", "notes", "weather"];
            const name = typeof payload.name === "string" ? payload.name : "none";
            if (valid.includes(name)) {
                if (payload.corner === "bl" || payload.corner === "br") {
                    z.setDockCorner(payload.corner);
                }
