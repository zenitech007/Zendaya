...
// Coordinated dock/undock choreography. Used by MainScene.
// Animates opacity, scale, position, fog, and bg-dim together (blueprint \u00a76).
...
export function dockOrbShowMap({ orb, map }: DockTargets): gsap.core.Timeline {
  ...
  map.children.forEach(child => {
    if ((child as THREE.Mesh).material instanceof THREE.ShaderMaterial) {
      tl.add(tweenUniform((child as THREE.Mesh).material as THREE.ShaderMaterial, "uBlur", 0, DUR.scene, EASE.cinematic), 0);
    }
  });
...
export function undockOrbHideMap({ orb, map }: DockTargets): gsap.core.Timeline {
  const z = useZendaya.getState();
  const tl = gsap.timeline({ defaults: { duration: DUR.scene, ease: EASE.cinematic } });
  ...
  map.children.forEach(child => {
    if ((child as THREE.Mesh).material instanceof THREE.ShaderMaterial) {
      tl.add(tweenUniform((child as THREE.Mesh).material as THREE.ShaderMaterial, "uBlur", 20.0, DUR.base, "power2.in"), 0);
    }
  });
...
export function voiceActivatePulse(orb: THREE.Group): gsap.core.Timeline {
