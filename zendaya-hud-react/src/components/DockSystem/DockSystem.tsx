...
export default function DockSystem() {
  ...
  return (
    <motion.div
      ...
      animate={{ y: minimized ? 80 : 0, opacity: minimized ? 0 : 1 }}
      transition={{ duration: 0.55, ease: [0.16, 1, 0.3, 1] }}
      className="fixed bottom-4 left-1/2 -translate-x-1/2 z-30 flex gap-2 px-3 py-2 rounded-2xl backdrop-blur-md bg-white/5 border border-white/10"
    >
      {ITEMS.map((it) => (
        <DockButton key={it.id} id={it.id} label={it.label} />
      ))}
    </motion.div>
  );
}

function DockButton({ id, label }: { id: string; label: string }) {
  ...
  return (
    <button
      ...
      className="font-display text-xs tracking-widest uppercase px-3 py-1.5 rounded-xl text-zen-cyan/90 hover:text-white hover:bg-white/10 transition-colors"
    >
      {label}
    </button>
  );
