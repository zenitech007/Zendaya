"""
skills.agent — autonomous plan / act / observe / replan loop.
...
def _act_write_file(args: Dict[str, Any]) -> str:
    ...
    try:
        import skills.coder
        return skills.coder.write_file_smart(desc, path)
    except Exception as e:
...
def _act_edit_file(args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        return skills.coder.edit_file_smart(path, change, preview=False)
    except Exception as e:
...
def _act_generate_project(args: Dict[str, Any]) -> str:
    ...
    try:
        import skills.coder
        return skills.coder.generate_project(spec, root)
    except Exception as e:
...
def _act_run(args: Dict[str, Any]) -> str:
    ...
    try:
        import skills.coder
        return skills.coder.run_code(path, timeout_s=timeout)
    except Exception as e:
...
def _act_grep(args: Dict[str, Any]) -> str:
    ...
    try:
        import skills.coder
        return skills.coder.grep_files(root, pattern, max_hits=int(args.get("max_hits", 60)))
    except Exception as e:
...
def _act_list_files(args: Dict[str, Any]) -> str:
    ...
    try:
        import skills.coder
        return skills.coder.list_files(root, max_entries=int(args.get("max_entries", 200)))
    except Exception as e:
...
def _act_syntax_check(args: Dict[str, Any]) -> str:
    ...
    try:
        import skills.coder
        ok, msg = skills.coder.syntax_check(path)
...
def _act_auto_fix(args: Dict[str, Any]) -> str:
    ...
    try:
        import skills.coder
        return skills.coder.run_with_autofix(path, max_attempts=attempts, timeout_s=timeout)
    except Exception as e:
...
def _act_bash(args: Dict[str, Any]) -> str:
    ...
    try:
        import skills.coder
        return skills.coder.safe_shell(command, cwd=cwd, timeout_s=timeout)
    except Exception as e:
...
def _act_verify(args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if check == "exists":
            import os
            ok = os.path.isfile(skills.coder._expand(path)) or os.path.isdir(skills.coder._expand(path))
        ...
        if check == "contains":
            ...
            try:
                with open(skills.coder._expand(path), "r", encoding="utf-8", errors="replace") as f:
                    text = f.read()
            except Exception as e:
        ...
        if check == "runs":
            ...
            result = skills.coder.run_code(path, timeout_s=int(args.get("timeout", 20)))
            import re as _re
...
def _act_ui_describe(args: Dict[str, Any]) -> str:
    ...
    try:
        import perception.uivision as uiv
...
def confirm_self_edit(pending: Dict[str, Any]) -> str:
    ...
    try:
        outcome = skills.coder.edit_file_smart(str(p), change, preview=False)
    except Exception as e:
...
if __name__ == "__main__":
    print("skills.agent loaded. Available actions:", sorted(ACTIONS.keys()))
